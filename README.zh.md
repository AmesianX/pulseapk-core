...
PulseAPK 采用单窗口工作流，并在顶部提供各工具导航：**Decompile**、**Build**、**Analyser**、**Settings** 和 **About**。每个部分对应 APK 生命周期的一个阶段，让你无需离开应用即可从解码切换到分析和签名。
...
4.  **.NET 8.0 Runtime**：在受支持的平台（Windows、Linux、macOS）上运行 PulseAPK 所需。
...
### 1) 反编译流程
![PulseAPK 反编译界面](images/pulse_apk_decompile.png)
- 该界面用于选择输入 APK 和输出目录，然后执行反编译。
- 简单流程：选择 APK -> 设置输出路径 -> 点击 decompile。

### 2) 构建流程
![PulseAPK 构建界面](images/pulse_apk_build.png)
- 该界面用于将反编译项目重新构建为新的 APK。
- 简单流程：选择项目目录 -> 设置输出名称/路径 -> 点击 build（需要时启用签名）。

### 3) 静态分析结果
![PulseAPK 分析输出](images/pulse_apk_analysis.png)
